...
//start new variables of page login (page=1 , duration=0, session-> goes to database of logs)
var duration = 0;
var page = 1;
...
// start counting time in each page
...
function login(e){
    ...
    if(value.length === 5){
        ...
        var xhttp = new XMLHttpRequest();
        xhttp.onreadystatechange = function (){
            if(this.readyState == 4 && this.status == 200){ // check status request is ok
                let res = JSON.parse(this.responseText);
                if(res["videos"].length > 0){ // found user code and than know the video sequence-there for video is not null!
        ...
        // put the code in back end or show error
